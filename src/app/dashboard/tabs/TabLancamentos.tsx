...
export default function TabLancamentos({ lancamentos, onSaveLancamento, onDeleteLancamento }: Props) {
  ...
  const fornecedoresHistorico = useMemo(()=>[...new Set(lancamentos.map(l=>l.fornecedor).filter(Boolean))],[lancamentos])
  ...
  async function handleSave(lancs:any[],editId?:number){
    ...
    toast(editId?'Atualizado ✓':lancs.length>1?`${lancs.length} lançamentos criados ✓`:'Lançamento adicionado ✓')
  }
    ...
    <div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px',flexWrap:'wrap',gap:'10px'}}>
        ...
        <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
          <button className="btn btn-s" onClick={()=>exportToExcel(lancamentos)}>⬇ Excel</button>
          <button className="btn btn-s" onClick={()=>exportSummary(lancamentos)}>📊 Resumo</button>
      ...
      <div style={{display:'flex',gap:'8px',flexWrap:'wrap',marginBottom:'16px'}}>
        <select value={filtroMes} onChange={e=>setFiltroMes(e.target.value)} className="filter-sel">
          <option value="">Todos os meses</option>
          {MESES.map(m=><option key={m} value={m}>{NOME_MES[m]}</option>)}
        </select>
        <select value={filtroTipo} onChange={e=>setFiltroTipo(e.target.value)} className="filter-sel">
          <option value="">Entrada/Saída</option>
          <option value="Entrada">Entrada</option>
          <option value="Saída">Saída</option>
        </select>
        <select value={filtroStatus} onChange={e=>setFiltroStatus(e.target.value)} className="filter-sel">
          <option value="">Todos status</option>
          <option value="Realizado">Realizado</option>
          <option value="A Realizar">A Realizar</option>
        </select>
        <select value={filtroBanco} onChange={e=>setFiltroBanco(e.target.value)} className="filter-sel">
          <option value="">Todos os bancos</option>
          {bancos.map(b=><option key={b} value={b}>{b}</option>)}
        </select>
        <input value={busca} onChange={e=>setBusca(e.target.value)} placeholder="Buscar..." className="filter-sel" style={{minWidth:'160px'}}/>
      </div>
      ...
      <LancamentoModal
        ...
        onSave={handleSave}
        fornecedoresHistorico={fornecedoresHistorico as string[]}
      />
    </div>
  )
}
