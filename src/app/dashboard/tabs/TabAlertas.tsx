...
interface Props {
  ...
  bancos: any[]; orcamento: any[]; saldosRef: any
...
export default function TabAlertas({ lancamentos, onSaveLancamento, onDeleteLancamento }: Props) {
  ...
  const renderList = (list: any[], cor: string) => {
    ...
    return list.map(l => {
      const diasAtraso = l.urgencia === 'vencido'
        ? Math.abs(Math.round((new Date(l.data + 'T00:00:00').getTime() - new Date().getTime()) / 86400000))
        : 0
      return (
        <div key={l.id} className="al-item" style={l.urgencia === 'vencido' ? { borderLeft: '3px solid var(--red)', paddingLeft: '10px' } : {}}>
          <span className={`bdg ${URG_MAP[l.urgencia as keyof typeof URG_MAP].cls}`} style={{ minWidth: '80px', justifyContent: 'center' }}>
            {URG_MAP[l.urgencia as keyof typeof URG_MAP].lbl}
          </span>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
        ...
        <select value={filtro} onChange={e => setFiltro(e.target.value)} style={{ background: 'var(--sur)', border: '1px solid var(--brd2)', borderRadius: 'var(--rs)', padding: '7px 12px', color: 'var(--txt)', fontSize: '13px', fontFamily: 'inherit', outline: 'none' }}>
