create table if not exists bancos_myfinance (
...
alter table bancos_myfinance enable row level security;
drop policy if exists "auth_all_bancos_mf" on bancos_myfinance;
create policy "auth_all_bancos_mf" on bancos_myfinance for all using (auth.role() = 'authenticated');

create table if not exists categorias_myfinance (
  id uuid primary key default gen_random_uuid(),
  nome text not null unique,
...
alter table categorias_myfinance enable row level security;
drop policy if exists "auth_all_categorias_mf" on categorias_myfinance;
create policy "auth_all_categorias_mf" on categorias_myfinance for all using (auth.role() = 'authenticated');

insert into bancos_myfinance (nome, saldo_abertura, data_abertura, classe, cor, ordem) values
...
insert into categorias_myfinance (nome, tipo, grupo, orcamento_default, ordem) values
